...
static uint32_t g_spp_conn_handle = 0;
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_CLOSE_EVT:
    	...
    	if (param->close.handle == g_spp_conn_handle) {
        	g_spp_conn_handle = 0; // сброс
    	}
    	break;
    ...
    case ESP_SPP_DATA_IND_EVT:
    	...
    	// Скопируем команду в null-терминированную строку (бд. что длина небольшая)
    	{
        	int len = param->data_ind.len;
        	int maxlen = (len < 512) ? len : 511;
        	char buf[512];
        	memset(buf, 0, sizeof(buf));
        	memcpy(buf, param->data_ind.data, maxlen);
        	// Удалим возможные \r\n
        	for (int i = 0; i < maxlen; i++) {
            	if (buf[i] == '\r' || buf[i] == '\n') { buf[i] = '\0'; break; }
        	}
        	ESP_LOGI(SPP_TAG, "Received payload: %s", buf);

        	// Простая команда идентификации: "ID"
        	if (strcasecmp(buf, "ID") == 0) {
            	const char *resp = "ESP32_TACHO;HW:DevKitC;VER:1;ID:0123\n";
            	if (g_spp_conn_handle != 0) {
    				esp_spp_write((uint32_t)g_spp_conn_handle, (int)strlen(resp), (uint8_t *)resp);
    				ESP_LOGI(SPP_TAG, "Sent identification response");
				} else {
    				ESP_LOGW(SPP_TAG, "No valid spp handle to respond");
				}
        	} else {
            // при необходимости обрабатывать другие команды
            	ESP_LOGI(SPP_TAG, "Unknown command received");
        	}
    	}
    	break;
    ...
    case ESP_SPP_SRV_OPEN_EVT:
    	...
    	// Сохраняем handle для дальнейшей esp_spp_write
    	g_spp_conn_handle = param->srv_open.handle;
    break;
